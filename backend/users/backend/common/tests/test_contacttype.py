...
class ContactTypeTestsCase(BaseTestView, TestCase):
    ...
    def setUp(self):
        self.conntacttype_1 = ContactType.objects.create(name="ContactType1")
        self.conntacttype_2 = ContactType.objects.create(name="ContactType1")
        super().setUp()
    ...
    def test_50_consumer_user_can_access_retrive(self):
        user = self.user
        model_id = self.conntacttype_1.id
        self.client.credentials(HTTP_AUTHORIZATION=self.get_token(user))
    ...
    def test_60_consumer_user_can_access_retrive(self):
        user = self.developer
        model_id = self.conntacttype_2.id
        self.client.credentials(HTTP_AUTHORIZATION=self.get_token(user))
        response = self.client.get(f"{self.url}{model_id}/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
