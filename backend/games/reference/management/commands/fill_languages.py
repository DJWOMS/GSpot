...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if Language.objects.filter(name__contains='test').exists():
            self.stdout.write('Test data Language already exists')
            return
        ...
        if ProductLanguage.objects.exists():
            self.stdout.write('Test data ProductLanguage already exists')
            return
