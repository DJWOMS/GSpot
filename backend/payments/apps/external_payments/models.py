...
from apps.payment_accounts.models import *
import uuid
...
class PaymentService(models.Model):
    name = models.CharField(max_length=30, unique=True, on_delete=models.PROTECT)

    def __str__(self):
        return f'{self.name}'


class PaymentComission(models.Model):
    payment_service_id = models.ForeignKey(PaymentService, on_delete=models.PROTECT)
    payment_type = models.CharField(max_length=50, verbose_name='type_of_payment')
    commission = models.FloatField(default=0, validators=[MinValueValidator(0, message='indicate the amount of commission')])
    date_time_creation = models.DateTimeField(auto_now_add=True, editable=False, db_index=True)

    def __str__(self):
        return (
            f'Type of payment: {self.payment_type}'
            f'Commission amount: {self.commission}'
            )
