...
class NotificationRepo:
    ...
    async def get_by_id(_id: str):
        return await db.your_mongodb_db.get_collection('notification').find_one({'_id': _id})

    @staticmethod
    async def get_user_notification(user_id: str):
        _notifications = []
        collection = db.your_mongodb_db.get_collection('notification').find({'user_id': user_id})
    ...
    async def update_status_notification(_id: str, status: str):
        await db.your_mongodb_db.get_collection('notification').update_one({'_id': _id}, {'$set': {'status': status}})
